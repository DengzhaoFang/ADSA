...
from ADSA import ADSA, create_adsa_from_config
from multimodal_dataset import ADSADataset, create_dataloaders
from adsa_losses import ADSATotalLoss
...
class ADSATrainer:
    """Main trainer class for ADSA with IDE + CMA support."""
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.setup_logging()
        self.logger.info("Initializing ADSA trainer...")
        ...
        self.logger.info("ADSA trainer initialized successfully")
    ...
    def setup_logging(self):
        ...
        self.logger = logging.getLogger('ADSATrainer')
    ...
    def setup_data(self):
        ...
        # PASA prior (Stage 2): precompute T(i,j) soft targets
        use_pasa = self.config.get('use_pasa', False)
        if use_pasa:
            import pandas as pd
            from pasa_prior import TopologySemanticPrior, build_item_neighbor_graph
            ...
            self.pasa_prior = TopologySemanticPrior(
            ...
            self.logger.info(f"  PASA prior: {len(item_df)} items, "
                             f"γ={self.config.get('text_sharpen_gamma',3.0)}, "
                             f"β={self.config.get('graph_scale_beta',0.05)}")
        else:
            self.pasa_prior = None
            self.item_pop = {}

    def setup_model(self):
        self.logger.info("Initializing ADSA model...")
        self.model = create_adsa_from_config(config=self.config)
    ...
    def setup_loss(self):
        ...
        use_pasa = self.config.get('use_pasa', False)
        use_cma = self.config.get('use_ide', True) and not use_pasa
        use_dual_head = self.config.get('use_dual_head', False)
        commit_w = self.config.get('commit_weight', 0.0625)

        # Validate mutual exclusivity
        if use_pasa:
            from pasa_loss import validate_mutual_exclusivity
            validate_mutual_exclusivity(use_pasa=True, use_cma=use_cma)
            from pasa_loss import PASA_Loss
            self.pasa_loss = PASA_Loss(
                temperature=self.config.get('pasa_temperature', 0.2),
                topk_K=self.config.get('pasa_topk', 5),
            ).to(self.device)
            self.logger.info(f"  PASA: ENABLED (replaces CMA)")
            self.logger.info(f"    τ={self.config.get('pasa_temperature', 0.2)} "
                             f"K={self.config.get('pasa_topk', 5)}")
            self.cma_loss = None  # CMA disabled
        else:
            self.pasa_loss = None

        self.loss_fn = ADSATotalLoss(
        ...
        align_name = 'PASA' if use_pasa else ('CMA' if use_cma else 'None')
    ...
    def train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            # PASA: replaces CMA with topology-aware soft contrastive loss
            if self.pasa_loss is not None:
                batch_ids_gpu = batch['item_id'].to(self.device)
                T = self.pasa_prior.compute_T(batch_ids_gpu)
                pop = self.item_pop_tensor[batch_ids_gpu]
                pa_loss, pa_dict = self.pasa_loss(h_t, h_c, T, pop)
                lambda_pa = self.config.get('lambda_pasa', 0.1)
                total_loss = total_loss + lambda_pa * pa_loss
                loss_dict.update(pa_dict)
                # Remove CMA from loss_dict if present (PASA replaces it)
    ...
    def train(self):
        self.logger.info("=" * 80)
        self.logger.info("Starting ADSA training")
        self.logger.info("=" * 80)
        ...
        for epoch in range(start_epoch, end_epoch + 1):
            ...
            if 'pasa' in train_metrics:
                self.logger.info(f"  PASA Loss: {train_metrics['pasa']:.4f}")
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train ADSA with IDE + CMA')
    ...
    # PASA arguments (replaces CMA)
    parser.add_argument('--use_pasa', action='store_true',
                        help='Enable PASA (asymmetric soft contrastive loss). '
                             'Mutually exclusive with CMA.')
    parser.add_argument('--pasa_temperature', type=float, default=0.2,
                        help='Temperature for PASA softmax (higher=softer P distribution)')
    parser.add_argument('--lambda_pasa', type=float, default=0.1,
                        help='Weight for PASA loss (default 0.1, same scale as CMA)')
    parser.add_argument('--pasa_topk', type=int, default=5,
                        help='Top-K truncation per row in PASA target (adaptive, relative constant)')
    parser.add_argument('--text_sharpen_gamma', type=float, default=3.0,
                        help='Text similarity sharpening exponent for PASA prior')
    parser.add_argument('--graph_scale_beta', type=float, default=0.05,
                        help='Graph similarity amplification threshold for PASA prior')
