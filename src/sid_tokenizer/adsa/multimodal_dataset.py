"""
Multi-Modal Dataset for ADSA Training
...
class ADSADataset(Dataset):
    ...
    def _sample_positive(self, item_id):
        import random
        cooc_list = self.cooc_graph.get(item_id, [])
        if len(cooc_list) == 0: return item_id
...
def create_dataloaders(
    ...
    max_items: Optional[int] = None,
    **dataset_kwargs
) -> Tuple[torch.utils.data.DataLoader, ADSADataset]:
    dataset = ADSADataset(
        data_dir=data_dir,
        max_items=max_items,
        **dataset_kwargs
