"""
Long-tail evaluation for generative recommender models (TIGER & Adsa).
"""
...
BEAUTY_PATHS = {
    ...
    'adsa': {
        ...
        'semantic_mapping_path': "scripts/output/adsa_tokenizer/beauty/3-256-32-ema-only-5-core-items/semantic_id_mappings.json",
...
class LongTailEvaluator:
    ...
    def __init__(
        ...
        # Purified multimodal support for Adsa
        ...
        if use_trie_constraints:
            from src.recommender.adsa.trie_constrained_decoder import (
...
def plot_comparison_results(
    ...
    model_base_colors = {
        ...
        'Adsa': ['#F28E2B', '#F5B366'],  # Orange shades
    }
...
def load_model_and_mapper(model_type: str, checkpoint_path: str, device: str):
    ...
    elif model_type == 'adsa':
        from src.recommender.adsa.dataset import SemanticIDMapper
        from src.recommender.adsa.model import create_model
    
    else:
        raise ValueError(f"Unknown model type: {model_type}. Must be 'tiger' or 'adsa'")
    ...
    # Create model with training_config for Adsa (needed for fusion modules)
    if model_type == 'adsa':
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Long-tail evaluation for TIGER/Adsa on Beauty dataset"
    )
    
    parser.add_argument('--model_type', type=str, required=True,
                       choices=['tiger', 'adsa'],
                       help='Model type: tiger or adsa')
...
def main():
    ...
    # Load multimodal embeddings for Adsa
    ...
    # Check for Trie-constrained decoding (applies to both TIGER and Adsa)
    ...
    if args.model_type == 'adsa':
        if training_config and hasattr(training_config, 'use_multimodal_fusion') and training_config.use_multimodal_fusion:
            use_multimodal = True
            logger.info("Loading purified Stage 1 features for Adsa...")
            
            from src.recommender.adsa.dataset import (
