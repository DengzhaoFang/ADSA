...
def parse_args():
    ...
    parser.add_argument('--embed_mode', type=str, default='tiger',
                       choices=['tiger', 'adsa'],
                       help='Embedding mode: tiger (concat all) or adsa (separate categories)')
    ...
    parser.add_argument('--max_tags', type=int, default=5,
                       help='Maximum number of category tags for adsa mode')
...
def generate_adsa_embeddings(item_info, model, max_tags, logger):
    """ADSA mode: Separate embeddings for categories and other attributes
...
def generate_all_item_embeddings(args, output_path, logger):
    ...
    tag_info = None  # Only used for adsa mode
    if args.embed_mode == 'tiger':
        item_embeddings = generate_tiger_embeddings(item_info, model, logger)
    elif args.embed_mode == 'adsa':
        item_embeddings, tag_info = generate_adsa_embeddings(item_info, model, args.max_tags, logger)
    else:
    ...
    # Save tag information for adsa mode
    if args.embed_mode == 'adsa' and tag_info is not None:
...
def main():
    ...
    if args.embed_mode == 'adsa':
        logger.info(f"")
        logger.info(f"  Tag files (adsa mode only):")
