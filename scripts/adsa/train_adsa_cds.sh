...
# ADSA Stage 1 tokenizer training for Amazon CDs.
# Run from anywhere:
...
DATA_PATH="${DATA_PATH:-dataset/Amazon-CDs/processed/cds-adsa-sentenceT5base/CDs}"
OUTPUT_DIR="${OUTPUT_DIR:-scripts/output/adsa_tokenizer/cds/hparam_stage1_PASCL/pasa_graph_dominant}"
DEVICE="${DEVICE:-cuda}"
...
case "$ALIGNMENT" in
    pasa)
        ALIGN_ARGS=(
            ...
            --pasa_topk "${PASA_TOPK:-7}"
            --text_sharpen_gamma "${TEXT_SHARPEN_GAMMA:-5.0}"
